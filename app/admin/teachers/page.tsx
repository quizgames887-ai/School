...
import { Users, Plus, Edit2, Trash2, Mail, Filter } from "lucide-react";
...
export default function TeachersPage() {
  ...
  const allGrades = useMemo(() => {
    ...
    teachersWithGrades.forEach((teacher: any) => {
      if (teacher.grades && teacher.grades.length > 0) {
        teacher.grades.forEach((grade: string) => gradeSet.add(grade));
      } else {
  ...
  const filteredTeachersByGrade = useMemo(() => {
    ...
    // Filter by subject within each grade
    if (filterSubject) {
      Object.keys(filtered).forEach((grade) => {
        filtered[grade] = filtered[grade].filter((teacher: any) =>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      {(allGrades.length > 0 || subjects.length > 0) && (
          ...
          {grades.length > 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredTeachersByGrade[grade].map((teacher: any) => (
            ...
            <Card>
              <CardContent className="p-12 text-center">
                <p className="text-sm text-gray-500">No teachers found matching the selected filters.</p>
              </CardContent>
            </Card>
          )}
        </div>
